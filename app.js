...
/* ===== Version 9: multiple archive photos per POI ===== */
window.__fr2026PendingPhotos = window.__fr2026PendingPhotos || [];

async function v9FilesToDataUrls(fileList){
  const files = Array.from(fileList || []);
  const results = [];
  for(const file of files){
    if(typeof fileToSmallDataUrl === "function"){
      results.push(await fileToSmallDataUrl(file));
    }
  }
  return results;
}
function v9RenderPhotoPreview(){
  const preview = document.getElementById("photoPreview");
  const photos = window.__fr2026PendingPhotos || [];
  if(!preview) return;
  if(!photos.length){
    preview.hidden = true;
    preview.innerHTML = "";
  ...
  preview.hidden = false;
  preview.innerHTML = `<div class="photo-preview-grid">${photos.map(p => `<img src="${p}" alt="Vorschau">`).join("")}</div>`;
}
function v9PatchPhotoInputs(){
  const archive = document.getElementById("poiPhotoArchive");
  const camera = document.getElementById("poiPhoto");
  if(!archive || archive.__v9Ready) return false;
  archive.__v9Ready = true;
  archive.setAttribute("multiple", "multiple");

  archive.addEventListener("change", async () => {
    const photos = await v9FilesToDataUrls(archive.files);
    window.__fr2026PendingPhotos = photos;
    window.__fr2026PendingArchivePhoto = photos[0] || null;
    v9RenderPhotoPreview();
  });

  if(camera && !camera.__v9Ready){
    camera.__v9Ready = true;
    camera.addEventListener("change", async () => {
      const photos = await v9FilesToDataUrls(camera.files);
      window.__fr2026PendingPhotos = photos;
      window.__fr2026PendingArchivePhoto = photos[0] || null;
      v9RenderPhotoPreview();
    });
...
function v9PatchPoiSubmit(){
  const form = document.getElementById("poiForm");
  if(!form || form.__v9MultiPhotoPatched) return false;
  form.__v9MultiPhotoPatched = true;

  form.addEventListener("submit", () => {
    setTimeout(() => {
      const photos = window.__fr2026PendingPhotos || [];
      if(photos.length && state.customPois?.length){
        const last = state.customPois[state.customPois.length - 1];
        if(last){
          last.photos = photos;
          last.photo = last.photo || photos[0];
          window.__fr2026PendingPhotos = [];
          window.__fr2026PendingArchivePhoto = null;
          save();
          if(typeof renderCustomPois === "function") renderCustomPois();
          v9RenderPhotoPreview();
...
function v9PhotoGridHtml(poi){
  const photos = poi.photos && poi.photos.length ? poi.photos : (poi.photo ? [poi.photo] : []);
  if(!photos.length) return "";
  return `<div class="poi-photo-grid">${photos.map(p => `<img src="${p}" alt="POI-Foto">`).join("")}</div>`;
}
const originalRenderCustomPoisV9 = typeof renderCustomPois === "function" ? renderCustomPois : null;
if(originalRenderCustomPoisV9){
  renderCustomPois = function(){
    originalRenderCustomPoisV9();
    const list = document.getElementById("poiList");
    if(!list || !state.customPois?.length) return;
    // Enhance list items that only show first photo by adding gallery data is hard to patch structurally,
    // so we re-render complete list in current filter/search state.
    const filter = document.getElementById("poiFilter")?.value || "Alle";
    ...
    list.innerHTML = pois.slice().reverse().map(p => `
      <div class="poi-item">
        <div class="poi-head">
          <div class="poi-symbol">${typeof categorySymbol === "function" ? categorySymbol(p.category) : "🚙"}</div>
          <div>
            <div class="poi-title">${p.name}</div>
            <div>${p.category} · <span class="stars">${typeof stars === "function" ? stars(p.rating) : ""}</span></div>
          </div>
        </div>
        ${v9PhotoGridHtml(p)}
        <div>${new Date(p.createdAt).toLocaleString("de-DE")} · ${Number(p.lat).toFixed(5)}, ${Number(p.lon).toFixed(5)}</div>
        ...
        <div class="poi-actions">
          <a class="link-btn secondary" target="_blank" rel="noopener" href="${mapsNavUrl(p.lat, p.lon)}">Navigation</a>
          <button class="danger-btn" onclick="deleteCustomPoi('${p.id}')">Löschen</button>
        </div>
      </div>
    `).join("");
  };
}
(function initV9MultiPhoto(){
  let tries = 0;
  const t = setInterval(() => {
    tries++;
    v9PatchPhotoInputs();
